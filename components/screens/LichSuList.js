...
const LichSuList = () => {
  ...
  const [filteredHoaDonData, setFilteredHoaDonData] = useState([]);
  ...
  useEffect(() => {
    const filteredData = hoadonData.filter(item => item.soDT === sdt);
    setFilteredHoaDonData(filteredData);
  }, [hoadonData, sdt]);
